	...
	switch($_GET['case'])
	{
		case 'CD':
			$sql = "SELECT `year`,`chemical_disease`.`PubMedIDs` 
					...
					WHERE `chemical_disease`.`ChemicalID` = '$_GET[search1]'
					AND	`chemical_disease`.`DiseaseID` = '$_GET[search2]'
		...
		case 'GD':
			$sql = "SELECT `year`,`gene_disease`.`PubMedIDs`
					...
					WHERE `gene_disease`.`GeneID` = '$_GET[search1]'
					AND `gene_disease`.`DiseaseID` = '$_GET[search2]'
		...
		case 'CG':
			$sql = "SELECT `year`,`chemical_gene`.`PubMedIDs`
					...
					WHERE `chemical_gene`.`ChemicalID` = '$_GET[search1]'
					AND `chemical_gene`.`GeneID` = '$_GET[search2]'
				...
				echo "<tr>";
					...
					echo "<td><a href='#' data-toggle='popover' data-placement='right' data-content='";
						echo "<table border=1>";
							...
							echo "<tr>";
								echo "<td><a href=grapheIndex.php?_return=".str_replace("'",'`',$temp[Chemical][$i][ChemicalID])." target=_blank>".str_replace("'",'`',$temp[Chemical][$i][ChemicalName])."</a></td>";
								echo "<td><a href=grapheIndex.php?_return=".str_replace("'",'`',$temp[Disease][$i][DiseaseID])." target=_blank>".str_replace("'",'`',$temp[Disease][$i][DiseaseName])."</a></td>";
								echo "<td><a href=grapheIndex.php?_return=".str_replace("'",'`',$temp[Gene][$i][GeneID])." target=_blank>".str_replace("'",'`',$temp[Gene][$i][GeneSymbol])."</a></td>";
							echo "</tr>";
							}
						echo "</table>'>";
					echo "點我</a></td>";
