<?php
		...
		{
			return "SELECT `chemical_disease`.`DiseaseID`,`DiseaseName` 
					FROM `chemical_disease`,`disease`
					WHERE `ChemicalID`='{$ChemicalID}'
					AND `chemical_disease`.`DiseaseID` = `disease`.`DiseaseID`
					Group by `chemical_disease`.`DiseaseID`";
		}
		function Chemical_Gene($ChemicalID)
		{
			return "SELECT `chemical_gene`.`GeneID`,`GeneSymbol`
					FROM `chemical_gene`,`gene` 
					WHERE `ChemicalID` = '{$ChemicalID}'
					AND `chemical_gene`.`GeneID` = `gene`.`GeneID`
					Group by `chemical_gene`.`GeneID`";
		}
		function Disease_Chemical($DiseaseID)
		{
			return "SELECT `chemical_disease`.`ChemicalID`,`ChemicalName` 
					FROM `chemical_disease` ,`chemical`
					WHERE `DiseaseID`='{$DiseaseID}'
					AND `chemical_disease`.`ChemicalID` = `chemical`.`ChemicalID`
					Group by `chemical_disease`.`ChemicalID`";
		}
		function Disease_Gene($DiseaseID)
		{
			return "SELECT `gene_disease`.`GeneID`,`GeneSymbol` 
					FROM `gene_disease`,`gene` 
					WHERE `DiseaseID`= '{$DiseaseID}'
					AND `gene_disease`.`GeneID` = `gene`.`GeneID`
					Group by `gene_disease`.`GeneID`";
		}
		function Gene_Chemical($GeneID)
		{
			return "SELECT `chemical_gene`.`ChemicalID`,`ChemicalName`
					FROM `chemical_gene`,`chemical` 
					WHERE `GeneID` = '{$GeneID}'
					AND `chemical_gene`.`ChemicalID` = `chemical`.`ChemicalID`
					Group by `chemical_gene`.`ChemicalID`";
		}
		function Gene_Disease($GeneID)
		{
			return "SELECT `gene_disease`.`DiseaseID`,`DiseaseName`
					FROM `gene_disease`,`disease`
					WHERE `GeneID` = '{$GeneID}'
					AND `gene_disease`.`DiseaseID` = `disease`.`DiseaseID`
					Group by `gene_disease`.`DiseaseID`";
		}
